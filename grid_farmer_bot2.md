...
Recovery Turtle exit:
  ...
  - N-day low break = 일봉 종가가 최근 N일 최저가 아래로 이탈
...
**- 부제: 동적 자산 배분 엔진과 09시 05분 확정 종가·4시간봉 조기 진입 기반의 레이어 분리 청산 시스템 -**
...
#### A. 농부 확정 매수 필터 (오전 09시 05분 검증, AND 결합)
매일 오전 09시 05분, 직전일 최종 확정 종가와 지표를 바탕으로 아래 필터가 모두 만족(`AND`)될 때만 농부 확정 자금을 집행합니다.
...
> **농부 매수 체결 구조:** 농부 매수 **가격 도달 감시는 24시간**(`trailing_stop_bot`) 수행하되, 확정 자금 집행은 **09시 05분 확정 필터를 통과할 때만** 실행합니다. 단, v3부터 농부 1~2차는 V자 반등 대응을 위해 4시간봉 기준 **부분 조기 진입**을 허용합니다.

#### A-1. 농부 매수 안전 필터 보완 (v4)

기존 필터는 "확정 캔들의 품질"을 잘 평가하지만, 무손절 물타기 전략에서 가장 위험한 **구조적 하락장**과 **자유낙하 구간**을 충분히 차단하지 못합니다. 따라서 농부 매수는 기존 09:05 필터 앞단에 아래 안전 필터를 추가합니다.
...
#### C. 부분 조기 진입 + 09시 05분 확정 진입
09시 05분 확정 필터는 안전하지만 V자 반등에는 늦을 수 있습니다. 따라서 농부 1~2차에 한해 4시간봉 회복 신호를 이용한 부분 조기 진입을 허용합니다.
...
조기 진입 후 다음 09시 05분 확정 필터를 통과하지 못하면 잔여 농부 자금은 집행하지 않습니다. 농부 3차는 최종 방어선이므로 조기 진입을 금지하고 확정 조건만 허용합니다.

조기 진입분은 손절하지 않습니다. 다만 조기 진입 후 09:05 확정 필터가 실패하거나 하락 속도 필터가 악화되면 **잔여 조기/확정 매수분만 미집행**하고, 이미 체결된 조기 진입분은 농부 물량으로 추적합니다.
...
$$\text{통합 회복 포지션 청산 조건} = (\text{청산 대상 통합 회복 포지션 예상 순손익} > 0) \ \mathbf{AND} \ (\text{현재가} < \text{진입 후 최고가} - \text{trailing width} \ \mathbf{OR} \ \text{09시 05분 검증 시 전날 종가} < \text{MA5})$$
...
MA5 청산은 09:05 확정 일봉 기준을 정식 청산 트리거로 유지합니다. 다만 09:05 이후 완만한 추세 이탈을 빨리 감지하기 위해 4시간봉 보조 이탈 신호를 별도로 기록합니다.
...
본 전략은 일봉 확정 진입, 4시간봉 부분 조기 진입, 실시간 가격 감시, 계좌/주문 정합성 복구를 함께 사용합니다. 업비트에서 받아와야 하는 정보는 다음과 같습니다.
...
| 장기 일봉 데이터 | REST `/v1/candles/days` | 최근 200~230개 `trade_price` | MA200, MA200 기울기, 구조적 하락장 차단 |
...
* **`ma5_trend_bot.py` (일 1회 / 크론탭 `5 9 * * *`):**
  * 핵심 임무: 09:00 전날 일봉 확정 직후 깨어나, ① 직전 20일 일봉으로 N(ATR) 계산·저장, ② 최근 200일 이상 일봉으로 MA200/장기 추세/최근 3일 하락률을 계산, ③ 농부 확정 매수 필터(KRW 거래대금·종가 위치·1일/2일 연속 양봉·과열 투매 예외·장기 추세·하락 속도·방어력 포함)를 연산하여 잔여 농부 자금 집행 또는 신호 기록, ④ 전날 종가의 MA5 하회 여부로 통합 회복 포지션 청산 판단. 상태는 `bot_state.json`에 **원자적 쓰기**(임시파일 → rename).
...
| 2 | 업비트 데이터 수집 모듈 | 주문 없음 | WebSocket 현재가/호가, 일봉, 4시간봉, 잔고, 주문 가능 정보 수집 |
...
| 6 | 농부매수 신호봇 | 주문 없음 | -15% 가격 조건과 09:05 확정 필터를 신호로만 기록 |
...
| 9 | 농부 1~2차 4시간봉 부분 조기 진입 | 제한 ON | 조기 진입 비율과 09:05 잔여 확정 진입 검증 |
...
if ENABLE_FARMER_CONFIRMED_BUY=true:
  09:05 확정 필터 통과 시 FARMER_BUY 실행
...
* 농부 1~2차 부분 조기 진입은 V자 반등 대응력을 높이지만, 09시 05분 확정 필터만 사용하는 방식보다 가짜 반등에 노출될 수 있습니다. 농부 3차는 이 위험 때문에 조기 진입을 금지합니다.
...
장중 노이즈가 배제된 09시 05분 확정 데이터 제어, 4시간봉 기반 부분 조기 진입, [현재 자산 대비 역산 동적 자본 제어], 농부 3차까지 닫히는 15.8% 자금 설계, 레이어 분리 청산, 무손절 추세추종 원칙을 통해 하락장 방어와 상승장 수확을 모두 도모합니다. 세부 설계·미결 사항·검증 계획은 Design 문서(`docs/02-design/features/grid_farmer_bot_design.md`)를 따릅니다.
