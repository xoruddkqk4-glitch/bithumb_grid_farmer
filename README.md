...
## Current Farmer/Turtle Implementation Notes (2026-06-14)

- Farmer buy is paper-only in the current implementation. `ENABLE_FARMER_CONFIRMED_BUY=true` allows confirmed farmer buys to execute, but the farmer executor is fixed to `PaperOrderExecutor`, so farmer buys do not place real Bithumb orders.
...
- Recovery Turtle exit now has three trigger paths behind the same positive-profit gate:
  ...
  - `MA5_EXIT`: daily close below MA5
  - `N_DAY_LOW_BREAK`: daily close below the recent N-day low
...
현재는 실거래가 아닌 Paper Trading 중심으로 동작하며, 그리드 매수/매도 상태, 수익률, 로그, 전략 설정, 텔레그램 알림을 대시보드에서 확인하고 일부 설정을 조정할 수 있습니다.

## 현재 구현 상태

- Bithumb 공개 REST API 기반 현재가 조회
- Paper Trading 그리드 매수/매도 실행
...
- 그리드 매매 10개 layer/stage 단위 요약 알림
- 일일 요약 알림 기준값 저장 구조
...
- Bithumb 실거래 주문 완전 연동 및 계좌 잔고 기반 검증
- 부분 익절 TP1/TP2 실제 매도 실행 로직
...
실거래 기능을 추가하거나 켤 때는 반드시 소액 검증, 로그 확인, 주문/잔고 정합성 검증을 먼저 진행해야 합니다.
...
부분 익절 설정은 대시보드 `Strategy Adjustment`에서 켜고 끌 수 있으며, 기본값은 꺼짐입니다. 현재 코드는 설정 저장과 입력 비활성화까지 반영되어 있고, TP1/TP2 실제 매도 실행은 다음 단계 구현 대상입니다.
...
Bithumb 공개 API에서 현재가를 주기적으로 수집합니다.
...
Default loop interval: 3 seconds
